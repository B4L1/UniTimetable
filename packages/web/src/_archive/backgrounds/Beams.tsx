...
function extendMaterial<T extends THREE.Material = THREE.Material>(
    ...
    Object.entries(cfg.uniforms ?? {}).forEach(([key, u]) => {
        // Correctly handle uniforms by checking if it's already an object with value property or needs wrapping
        if (u !== null && typeof u === 'object' && 'value' in u) {
            uniforms[key] = u as THREE.IUniform;
        } else {
            uniforms[key] = { value: u };
        }
    });
...
const Beams: FC<BeamsProps> = ({
  ...
  vec3 getNormal(vec3 pos) {
    ...
    return normalize(cross(tangentZ, tangentX));
                ...
                //     gl_FragColor.rgb -= vec3(randomNoise / 15. * uNoiseIntensity);`
                // },
                material: { fog: true },
                uniforms: {
